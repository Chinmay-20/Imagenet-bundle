...
from meanreprocessor import MeanPreprocessor
import numpy as np
import mxnet as mx
import argparse
import pickle
...
for row in rows:
	...
	idxs=nnp.argsort(preds)[::-1][:5]
	
	print("[INFO] actual={}".format(le.inverse_transform(target)))
	
	
	label=lee.inverse_transform(idxs[0])
	label=label.replace(":"," ")
	...
	for (i,prob) in zip(idxs,preds:
		print("\t[INFO] predicted={},probablity={:.2f}%".format(le.inverse_transform(i),preds[i]*100))
